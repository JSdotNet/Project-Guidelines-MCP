---
title: "ADR 0010: Adopt OpenTelemetry for Comprehensive Observability"
date: 2025-11-26
status: Accepted
...
```csharp
public sealed class CreateOrderHandler : ICommandHandler<CreateOrderCommand, Result<CreateOrderResult>>
{
    ...
    public async Task<Result<CreateOrderResult>> Handle(CreateOrderCommand command, CancellationToken ct)
    {
        ...
        try
        {
            ...
            using (var validateActivity = ActivitySource.StartActivity("ValidateCustomer"))
            {
                ...
                if (customer is null)
                {
                    validateActivity?.SetStatus(ActivityStatusCode.Error, "Customer not found");
            ...
            using (var paymentActivity = ActivitySource.StartActivity("ProcessPayment"))
            {
                ...
                if (!paymentResult.Success)
                {
                    ...
                    return Result<CreateOrderResult>.Failure("payment.failed", paymentResult.ErrorMessage);
                }
            }
...
public async Task<Result<CreateOrderResult>> Handle(CreateOrderCommand command, CancellationToken ct)
{
    ...
    var result = await ExecuteCore(command, ct);

    _metrics.RecordProcessingDuration(stopwatch.Elapsed.TotalMilliseconds, success: result.IsSuccess);
    if (result.IsSuccess)
        _metrics.RecordOrderCreated(result.Value!.Total, "unknown");
...
- **Error rates**: Failed payments, validation errors, unexpected exceptions
...
1. **Guidelines enforcement**: Stylelint-style analyzer for OTel (future: custom Roslyn analyzer)
2. **Dashboards**: Provide Grafana/Aspire dashboard templates for common metrics
3. **Cost controls**: Monitor telemetry data volume; adjust sampling rates per environment (dev=100%, prod=10%)

## Integration with Existing ADRs

- **ADR 0003 (Aspire)**: Aspire includes OTel by default; use `AddServiceDefaults()`
- **ADR 0006 (CQRS)**: Handlers are natural activity boundaries; instrument all handlers
- **ADR 0008 (Vertical Slices)**: Each slice's handler emits telemetry independently
- **ADR 0004 (Result Objects)**: Telemetry should treat expected business failures as failed `Result` outcomes, reserving exception traces for unexpected faults
