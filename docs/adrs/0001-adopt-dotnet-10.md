...
Prefer `.slnx` for new and updated solution files. `.sln` remains allowed as a compatibility fallback for environments or tools that do not yet fully support `.slnx`; this choice does not alter the required target framework baseline defined by this ADR.
...
- Solution file formats (`.sln` and `.slnx`) documentation
