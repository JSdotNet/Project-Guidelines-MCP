...
Every overridden token must supply both a light-mode value and a dark-mode value. Partial overrides (for example, only light mode) are not permitted.
...
| Semantic surface token | 4.5:1 against its intended foreground text or icon color |

Use a contrast checker (for example, [WebAIM Contrast Checker](https://webaim.org/resources/contrastchecker/)) to validate before merging.

### 3. Semantic Surfaces Must Remain Distinct

Overriding a semantic color (for example, `color-success`) requires that:
- The semantic surface remains clearly distinguishable from `color-background`.
- The intended foreground text or icon color on that surface maintains at least 4.5:1 contrast.
...
Overrides are declared in the project's central token file, as specified in [ADR 0011](../guide/adrs/0011-centralized-frontend-styling-variables.md). The mechanism is framework-agnostic: replace the default value of only the permitted tokens and leave all others unchanged.
...
[Project override file - only brand or semantic colors]
  color-primary (light):  #8B0000   <- project brand red
  color-primary (dark):   #C0392B   <- accessible dark-mode variant
```

The implementation details (SCSS variable override, CSS custom property override, design-token JSON merge) depend on the frontend technology. See [ADR 0011](../guide/adrs/0011-centralized-frontend-styling-variables.md) for framework-specific patterns.

---

## Step-by-Step Override Process

1. **Identify brand colors.** Obtain the brand color palette from the design team (brand guidelines PDF, Figma file, and so on).
...
3. **Derive dark-mode variants.** For each brand color chosen for light mode, derive an accessible dark-mode variant.
...
6. **Create `COLOR_TOKENS.md`.** Document all values and ratios in the project repository.
...
|---|---|---|---|---|---|
| `color-primary` | `#A16207` | `#C0392B` | `#E74C3C` | 5.1:1 Yes | 4.6:1 Yes |
| `color-primary-light` | `#D4A72C` | `#E74C3C` | `#F07060` | - | - |
| `color-primary-dark` | `#7C4A03` | `#922B21` | `#C0392B` | - | - |
| `color-secondary` | `#6C757D` | `#7D3C98` | `#A569BD` | 4.6:1 Yes | 4.5:1 Yes |
