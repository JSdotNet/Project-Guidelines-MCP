...
public sealed class GitHubDocumentCatalog : IDocumentCatalog, IAsyncDisposable
{
    ...
    public async Task<string> GetContentAsync(string id, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var d in all)
        {
            ...
            {
                var relativePath = d.RelativePath.Replace("\\", "/");
                var rawUrl = $"https://raw.githubusercontent.com/{_owner}/{_repo}/{_branch}/{_documentsPath}/{relativePath}";
    ...
    {
        var cacheKey = $"docs-index:{_owner}/{_repo}/{_branch}:{_documentsPath}";
        ...
        try
        {
            var indexUrl = $"https://raw.githubusercontent.com/{_owner}/{_repo}/{_branch}/{_documentsPath}/index.json";
    ...
    private async Task<IReadOnlyList<DocumentInfo>> LoadIndexViaDirTraversalAsync()
    {
        ...
        await foreach (var file in EnumerateDocsAsync(_documentsPath).ConfigureAwait(false))
        {
            ...
            var title = file.name.EndsWith(".md", StringComparison.OrdinalIgnoreCase)
                ? file.name[..^3].Replace('-', ' ')
                : file.name;
            var prefix = _documentsPath + "/";
            var relative = file.path.StartsWith(prefix, StringComparison.OrdinalIgnoreCase)
            ...
            try
            {
                var rawUrl = $"https://raw.githubusercontent.com/{_owner}/{_repo}/{_branch}/{_documentsPath}/{relative}";
    ...
    private static string NormalizeDocumentsPath(string documentsPath)
    {
        ...
        return documentsPath.Trim().Trim('/').Replace('\\', '/');
    }
