---
title: "ADR 0008: Adopt Vertical Slice Architecture for Feature Organization"
date: 2025-11-24
status: Accepted
...
tests/ProjectName.Orders.UnitTests/
├── CreateOrder/
...
#### 1. Handler Pattern (CQRS-aligned, per ADR 0006)
...
public sealed class CreateOrderCommandHandler : ICommandHandler<CreateOrderCommand, Result<CreateOrderResult>>
{
    ...
    public async Task<Result<CreateOrderResult>> Handle(CreateOrderCommand command, CancellationToken ct)
    {
        ...
        var customer = await _customers.GetByIdAsync(command.CustomerId, ct)
            ;
        if (customer is null)
            return Result<CreateOrderResult>.Failure("customer.not_found", $"Customer {command.CustomerId} not found");
...
**Level 2 — Domain model (authoritative):** The handler creates or loads the domain model via a factory (`Order.Create(...)`) or a behavior method (`customer.UpdateProfile(...)`). The domain model enforces invariants; handlers translate expected domain rule violations into failed `Result` values at the application boundary. This is the definitive validation layer.
...
var order = Order.Create(command.CustomerId, command.Lines); // map expected domain violations to Result failures before returning
```
...
Tests live in a dedicated test project under root `tests/` that mirrors the feature structure of the module (see ADR 0009). Use **xUnit**, **Moq**, and **Bogus**:

```csharp
namespace ProjectName.Orders.UnitTests.CreateOrder;
...
public sealed class CreateOrderCommandHandlerTests
{
    ...
    public async Task Handle_ShouldReturnFailure_WhenCustomerDoesNotExist()
    {
        ...
        Assert.Equal("customer.not_found", result.ErrorCode);
    }
}
```
...
- **Cross-cutting concerns**: Logging, error mapping, authorization → middleware or base classes in `Core/`.
...
Vertical slices should use explicit request/handler contracts without depending on a mediator library.

```csharp
// Direct DI registration per handler
builder.Services.AddScoped<ICommandHandler<CreateOrderCommand, Result<CreateOrderResult>>, CreateOrderCommandHandler>();
```

If your solution already models requests with `IRequest<T>` / `IRequestHandler<TRequest, TResponse>` interfaces, keep them as local contracts in your application layer and resolve handlers through dependency injection.
...
- **ADR 0005 (Modular Monolith)**: Vertical slices can be organized into modules (bounded contexts) at a higher level.
- **ADR 0006 (CQRS)**: Vertical slices naturally align with command/query handlers.
- **ADR 0007 (Minimal APIs)**: Thin endpoint mapping delegates to slice handlers.
- **ADR 0004 (Result Objects)**: Expected business outcomes should cross application boundaries as `Result`/`Result<T>`.
- **ADR 0009 (Feature Slices Module Structure)**: For modular-monolith projects, ADR 0009 **supersedes** the physical project layout described in this ADR. ADR 0009 defines the authoritative folder structure within a module, including `Features/`, `DomainModels/`, `Observability/`, `Services/`, and the handler naming convention (`{Feature}CommandHandler` / `{Feature}QueryHandler`). Tests are in **separate** test projects under root `tests/`, not co-located.
...
tests/ProjectName.Customers.UnitTests/UpdateCustomerProfile/
...
public sealed class UpdateCustomerProfileCommandHandler : ICommandHandler<UpdateCustomerProfileCommand, Result>
{
    private readonly ICustomerRepository _customers;
    ...
    public async Task<Result> Handle(UpdateCustomerProfileCommand command, CancellationToken ct)
    {
        var customer = await _customers.GetByIdAsync(command.CustomerId, ct)
            ;
        if (customer is null)
            return Result.Failure("customer.not_found", $"Customer {command.CustomerId} not found");

        // Domain model enforces business rules; handler maps expected violations to failed results
