---
title: "ADR 0007: Recommend Minimal APIs Over Controller-Based APIs"
date: 2025-11-10
status: Accepted
...
- Keep endpoint lambdas thin; delegate to handlers (see ADR 0006 CQRS recommendation).
- Prefer `Result`/`Result<T>` at handler boundaries for expected outcomes (see ADR 0004), and map them to HTTP results in endpoints.
- Use C# records for request/response models.
- Apply **shallow validation** at the edge via endpoint filters or manual checks (required fields, format, range). Shallow validation catches malformed requests early and returns `400 Bad Request`. It cannot enforce business invariants — domain model validation (see ADR 0008) is authoritative.
...
orders.MapPost("/", async (CreateOrderRequest req, ICommandHandler<CreateOrderCommand, Result<CreateOrderResult>> handler, CancellationToken ct) =>
{
...
- Extract business logic from controller actions into handlers (see ADR 0006).
